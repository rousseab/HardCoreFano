import common
reload(common)
from common import *

#----------------------------------------
# import modules
#----------------------------------------
import matplotlib as mpl
import matplotlib.pyplot as plt
from matplotlib.font_manager import  FontProperties

import matplotlib.cm as cm
import matplotlib.tri as tri

#================================================================================
# Compute I
#================================================================================
ihw       = N.int(150)

#q_vector = N.random.random(2)

list_hw  = N.arange(0.,300.,1.)/1000. # in eV

T = 300. # K
beta = 1./(kB*T)
mu   = -0.400 # eV

delta_width = 0.050 # eV


#nmax = 12
#grid = TesselationGrid(nmax)

nmax_coarse = 8
nmax_fine   = 32
n_blocks_coarse_to_fine = 3

include_Gamma =True
grid = TesselationDoubleGrid(nmax_coarse, nmax_fine, n_blocks_coarse_to_fine,include_Gamma )


hw       = list_hw[ihw]

k_mu     = N.abs(mu)/hvF
k_w      = N.abs(hw)/hvF

K = twopia*2./3*N.array([1.,0.])
q_vector = k_mu*N.array([1.0,0.00])+K


fig1 = plt.figure(figsize=(12,10))
fig2 = plt.figure(figsize=(12,10))

ax1  = fig1.add_subplot(111)
ax2  = fig2.add_subplot(111)

list_fig = [fig1,fig2]
list_ax  = [ax1,ax2]

for ax in list_ax:
    ax.set_xlabel('$k_x$ ($2\pi/a$)')
    ax.set_ylabel('$k_y$ ($2\pi/a$)')

mymap = plt.get_cmap("spectral")

list_min = []
list_max = []

list_images = []

for i,wedge in enumerate(grid.list_wedges):
        
    I = IGridFunction( q_vector,list_hw, delta_width, mu, beta, wedge)

    index_I = 0
    S_im  = N.imag(I.I[index_I,:,ihw])
    S_re  = N.real(I.I[index_I,:,ihw])
    S_abs = N.abs(I.I[index_I,:,ihw])

        x = wedge.list_k[:,0]/twopia
        y = wedge.list_k[:,1]/twopia

    t = wedge.triangles_indices

    fc = S_re[t].mean(axis=1)
    image = ax1.tripcolor(x,y,triangles=t,facecolors=fc,cmap=plt.cm.spectral_r,  edgecolors='w')
    list_images.append(image)

    fc = S_im[t].mean(axis=1)
    image = ax2.tripcolor(x,y,triangles=t,facecolors=fc,cmap=plt.cm.spectral_r,  edgecolors='w')
    list_images.append(image)


    fc = S_abs[t].mean(axis=1)
    list_min.append(N.min(fc))
    list_max.append(N.max(fc))

        #image = ax.tripcolor(x,y, S, triangles=t, shading='gouraud', edgecolors='k',cmap=plt.cm.spectral_r)

vmin = N.min(list_min)
vmax = N.max(list_max)

norm = mpl.colors.Normalize(vmin=vmin, vmax=vmax)
for image in list_images:
        image.set_norm(norm)



fig1.colorbar(image,ax=ax1)
fig2.colorbar(image,ax=ax2)

#fig.colorbar(image,ax=list_ax[1])


#axc = plt.colorbar(image)
#axc.set_clim(vmin=S_min,vmax=S_max)

c = N.cos(N.pi/3.)
s = N.sin(N.pi/3.)
list_centers = N.array([ [0.,0.], [-2./3.,0.], [ 2./3.,0.], 2./3.*N.array([c,s]),
                       2./3.*N.array([c,-s]), 2./3.*N.array([-c,s]), 2./3.*N.array([-c,-s])])

rad_max = (2*k_mu+k_w)/twopia
rad_mid = (2*k_mu-k_w)/twopia
rad_min = k_w/twopia

rad_F = k_mu/twopia
list_centers_F = N.array([ [-2./3.,0.], [ 2./3.,0.], 2./3.*N.array([c,s]),
                       2./3.*N.array([c,-s]), 2./3.*N.array([-c,s]), 2./3.*N.array([-c,-s])])


th = N.arange(0.,2.*N.pi,0.01)

for center in list_centers_F:

    x = center[0]+rad_F*N.cos(th)
    y = center[1]+rad_F*N.sin(th)

    for ax in list_ax:
        ax.plot(x, y,'g-',lw=2)


for center in list_centers:

    x = center[0]+rad_max*N.cos(th)
    y = center[1]+rad_max*N.sin(th)

    for ax in list_ax:
        ax.plot(x, y,'k-',lw=2)

    x = center[0]+rad_min*N.cos(th)
    y = center[1]+rad_min*N.sin(th)

    for ax in list_ax:
        ax.plot(x, y,'k-',lw=2)


for ax in list_ax:
    for line in ax.xaxis.get_ticklines()+ax.yaxis.get_ticklines():
        line.set_markeredgewidth(3)


for ax in list_ax:
    ax.set_xlim([-0.7,0.7])
    ax.set_ylim([-0.7,0.7])

fig1.suptitle('Real part')
fig2.suptitle('Imaginary part')


for fig in list_fig:
    fig.gca().set_aspect('equal')



    fig.subplots_adjust(    left    =       0.16,
                bottom  =       0.16,
                right   =       0.96,
                top     =       0.98,
                wspace  =       0.20,
                hspace  =       0.20)


plt.show()
