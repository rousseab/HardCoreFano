...
ax2.set_title('Real Part KK integral')
ax3.set_title('Imaginary Part KK integral')



ax1.set_ylabel('Im[F$(\omega)$]')
ax2.set_ylabel('Re[KKF$(\omega)$]')
ax3.set_ylabel('Im[KKF$(\omega)$]')
...
list_exact_KK   = complex(1.,0.)/(list_hw+1j*Gamma+1j*delta)

#list_Fr_exact = N.real(list_F_exact )

list_KK_F = KKG.apply_kramers_kronig_FFT_convolution(list_localized_kernel)

ax1.plot(list_hw,N.imag(list_localized_kernel),'r-',lw=lw,label='Lorentzian')


ax2.plot(list_hw,N.real(list_KK_F ),'r-',lw=lw,label='Kramers-Kronig')
ax2.plot(list_hw,N.real(list_exact_KK),'g--',lw=lw,label='exact')


ax3.plot(list_hw,N.imag(list_KK_F),'r-',lw=lw,label='Kramers-Kronig')
ax3.plot(list_hw,N.imag(list_exact_KK),'g--',lw=lw,label='exact')
